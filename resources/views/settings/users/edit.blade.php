...
<div class="p-6">
        ...
        <form id="edit-user-form" action="{{ route('users.update', $user->id) }}" method="POST" class="space-y-4">
            ...
            <div id="face-scan-container" class="border border-[#E4E4E7] p-4 rounded-md bg-[#F4F4F5] mb-4 transition-all duration-300 shadow-sm">
                <label class="block text-xs font-bold text-[#71717A] uppercase tracking-wider mb-2">
                    Pembaruan Wajah (Webcam Scan) 
                    @if(!$user->face_photo)
                        <span class="text-[#B91C1C] font-bold">* Wajib</span>
                    @else
                        <span class="text-[#71717A] font-normal">(Opsional, sudah terdaftar)</span>
                    @endif
                </label>
                ...
                <div class="flex flex-col items-center gap-3">
                    @if($user->face_photo)
                        <div class="mb-3 text-center">
                            <p class="text-xs text-[#71717A] mb-1">Wajah Terdaftar Saat Ini:</p>
                            <img src="{{ asset('storage/' . $user->face_photo) }}" class="w-32 h-32 object-cover rounded-full border border-[#E4E4E7] shadow-sm mx-auto">
                        </div>
                    @endif
            ...
            <div class="relative w-64 h-64 bg-[#FFFFFF] rounded-md overflow-hidden border border-dashed border-[#E4E4E7] flex items-center justify-center mx-auto shadow-inner">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    <label class="block text-xs font-bold text-[#71717A] uppercase tracking-wider mb-2">Password Baru</label>
                ...
                <div>
                    <label class="block text-xs font-bold text-[#71717A] uppercase tracking-wider mb-2">Konfirmasi Password</label>
    ...
    <script>
        ...
        // Data karyawan terdaftar untuk validasi duplikasi wajah (kecuali user yang diedit)
        const existingUsers = [
        ...
        btnCapture.addEventListener('click', () => {
            ...
            scanInterval = setInterval(() => {
                ...
                if (duplicateFound && maxSimilarity >= threshold) {
                    ...
                    statusOverlay.className = "absolute bottom-4 left-1/2 -translate-x-1/2 bg-[#FEE2E2]/90 border border-red-400/40 backdrop-blur-sm rounded-full px-3 py-1 text-[9px] font-extrabold tracking-widest text-[#B91C1C] uppercase z-20 shadow-lg";
                    laser.classList.add('hidden');
                    
                    alert(`⚠️ Pembaruan Wajah Ditolak!\n\nWajah yang dideteksi sangat mirip dengan karyawan lain (${duplicateFound.name} - ${maxSimilarity.toFixed(1)}% Match).\n\nMohon pastikan wajah yang didaftarkan unik dan bukan milik karyawan lain.`);
        ...
        const hasExistingFace = {{ $user->face_photo ? 'true' : 'false' }};
        
        form.addEventListener('submit', (e) => {
            if (roleSelect && roleSelect.value === 'admin') {
                return; // Bypass validation for admin role
            }
            if (!hasExistingFace && !facePhotoInput.value) {
                e.preventDefault();
                alert("⚠️ Pendaftaran Wajah Wajib! User ini belum memiliki foto wajah terdaftar. Harap aktifkan kamera dan lakukan pemindaian wajah terlebih dahulu sebelum menyimpan.");
